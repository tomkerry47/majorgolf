...
import { golfers, type InsertGolfer } from '../shared/schema.js'; // Added .js extension
import { sql, eq, ilike } from 'drizzle-orm'; // Import eq and ilike

const DATAGOLF_RANKINGS_URL = 'https://datagolf.com/datagolf-rankings';

// Helper function to create short name (e.g., R. McIlroy)
function createShortName(firstName: string, lastName: string): string | null {
...
async function updateGolfersFromDataGolf() {
  ...
  try {
    ...
    const existingGolferMap = new Map<string, { id: number; rank: number; shortName: string | null; firstName: string | null; lastName: string | null }>();
    // Add explicit type for 'g' based on the select statement
    existingGolfers.forEach((g: { id: number; name: string | null; rank: number; shortName: string | null; firstName: string | null; lastName: string | null }) => {
    ...
    const golfersToInsert: InsertGolfer[] = [];
    const golfersToUpdate: { id: number; rank: number; shortName: string | null; firstName: string | null; lastName: string | null }[] = [];
    let skippedCount = 0;
