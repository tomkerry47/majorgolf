...
const getRows = (result) => result?.rows || []; // Added constraint
// Modified to accept the connection pool directly with type annotation
export async function updateResultsAndAllocatePoints(pool, competitionIdToProcess = null) {
    ...
    try {
        ...
        if (competitionIdToProcess !== null) {
            ...
            const recentCompRes = await client.query('SELECT * FROM competitions WHERE "isComplete" = true AND "endDate" >= $1 ORDER BY "endDate" DESC', [oneMonthAgo]);
...
async function fetchAndProcessPgaData(client, competition) {
    ...
    try {
        ...
        const posColumn = jsonData.find((col) => col['csvw:name'] === 'POS');
        const playerColumn = jsonData.find((col) => col['csvw:name'] === 'PLAYER');
        const scoreColumn = jsonData.find((col) => col['csvw:name'] === 'TOT'); // Total score
        const thruColumn = jsonData.find((col) => col['csvw:name'] === 'THRU'); // To check for completion
        ...
        const isTournamentComplete = thruColumn['csvw:cells'].every((cell) => cell['csvw:value'] === 'F' || cell['csvw:value'] === '-');
...
async function processCompetition(client, competition) {
...
async function completeCompetition(client, competitionId) {
...
async function allocatePoints(client, competitionId) {
    console.log(`Allocating points for competition ${competitionId}...`);
    try {
        // 1. Get results (ensure points column is selected)
        const resultsRes = await client.query('SELECT id, "competitionId", "golferId", position, score, points FROM results WHERE "competitionId" = $1 ORDER BY position ASC', [competitionId]);
        let results = getRows(resultsRes); // Use let as we might update it
        if (results.length === 0) {
        ...
        const ranksRes = await client.query('SELECT "userId", "golferId", "rankAtDeadline" FROM selection_ranks WHERE "competitionId" = $1', [competitionId]);
        const selectionRanksData = getRows(ranksRes);
        const rankMap = new Map(); // Key: "userId-golferId", Value: rank
        ...
        for (const selection of selections) {
            let totalPoints = 0;
            const pointDetails = [];
            const golferIds = [selection.golfer1Id, selection.golfer2Id, selection.golfer3Id].filter((id) => Boolean(id)); // Type guard
            ...
            for (const golferId of golferIds) {
                ...
                let basePoints = 0;
                let position = 'N/A'; // Default position if no result
                if (result) {
